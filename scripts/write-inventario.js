// Script: escribe InventarioPage.tsx con UTF-8 correcto
const fs   = require('fs');
const path = require('path');
const dest = path.join(__dirname, '../apps/web/src/modules/inventario/InventarioPage.tsx');
...
const CATEGORIAS: Record<Categoria, { label: string; emoji: string; color: string }> = {
  ...
  clinico:     { label: 'Material cl${iacute}nico', emoji: '🩺', color: 'bg-cyan-50  border-cyan-200  text-cyan-700'  },
          ...
          {tab === 'documento' && (
            <div className="space-y-5">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {([
                    { v: 'boleta',     l: '🧾 Boleta'        },
                    { v: 'factura',    l: '📄 Factura'       },
                    { v: 'nota_debito',l: '📋 Nota de d${eacute}bito' },
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <Field label="Direcci${oacute}n proveedor">
                  <input value={form.proveedorDireccion} onChange={e => set({ proveedorDireccion: e.target.value })}
                    placeholder="Direcci${oacute}n fiscal" className={inputCls} />
                </Field>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button onClick={addItem} className="text-xs text-emerald-700 font-medium hover:text-emerald-800">+ Agregar l${iacute}nea</button>
                </div>
                ...
                <div className="rounded-xl border border-gray-100 overflow-hidden">
                  <div className="grid grid-cols-12 gap-2 px-3 py-2 bg-gray-50 text-[10px] font-semibold text-gray-400 uppercase tracking-wide border-b border-gray-100">
                    ...
                    <span className="col-span-2">C${oacute}digo</span>
          ...
          {tab === 'producto' && (
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <Field label="C${oacute}digo interno">
                  <input value={form.codigo} onChange={e => set({ codigo: e.target.value })}
                    placeholder="Ej: MED-001" className={inputCls} />
                </Field>

                <Field label="Categor${iacute}a" required>
              ...
              <Field label="Descripci${oacute}n / Observaciones">
                <textarea value={form.descripcion} onChange={e => set({ descripcion: e.target.value })}
                  rows={3} placeholder="Descripci${oacute}n del producto, indicaciones, precauciones${hellip}"
          ...
          {tab === 'stock' && (
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <Field label="Stock m${iacute}nimo (alerta)">
                  <div className="relative">
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex flex-col justify-center">
                  ...
                  <p className="text-xs text-amber-600 mt-0.5">
                    Se activar${aacute} cuando el stock caiga a {form.stockMinimo} {form.unidad}(s) o menos.
                  </p>
                </div>
              </div>
          ...
          <div className="flex gap-2">
            ...
            {tab !== 'stock' ? (
              ...
              <button
                ...
                disabled={!canSaveProd || !canSaveDoc}
                className="px-5 py-2 rounded-lg text-sm font-semibold bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-40 transition-colors shadow-sm"
              >
                ✓ Registrar producto
              </button>
            )}
...
export function InventarioPage() {
  ...
  function guardar(form: Omit<Producto,'id'>) {
    if (editProd) {
      setProductos(p => p.map(x => x.id === editProd.id ? { ...form, id: editProd.id } : x));
    } else {
      setProductos(p => [...p, { ...form, id: Date.now() }]);
    }
    setShowModal(false);
    setEditProd(null);
  }
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="grid grid-cols-12 gap-3 px-5 py-3 bg-gray-50 border-b border-gray-100 text-xs font-semibold text-gray-500 uppercase tracking-wider">
          <span className="col-span-4">Producto</span>
          <span className="col-span-2">Categor${iacute}a</span>
        ...
        {filtrados.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-14 text-center">
            ...
            <p className="text-xs text-gray-400 mt-1">
              {search || filtrocat ? 'Cambia los filtros' : 'Registra el primer producto con el bot${oacute}n superior'}
            </p>
          </div>
        ) : (
          filtrados.map(p => {
...
console.log('InventarioPage.tsx OK (' + content.length + ' bytes)');
