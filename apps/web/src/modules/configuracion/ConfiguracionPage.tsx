...
function SeccionDocumentos() {
  ...
  const DOCS: { tipo: TipoDocumento; label: string; desc: string }[] = [
    { tipo: 'boleta',       label: 'Boleta',         desc: 'Imagen de encabezado para boletas de venta' },
    { tipo: 'factura',      label: 'Factura',         desc: 'Imagen de encabezado para facturas' },
    { tipo: 'nota_credito', label: 'Nota de Crédito', desc: 'Imagen de encabezado para notas de crédito' },
  ];
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-gray-400 mt-0.5">Sube la imagen de membrete/encabezado que aparecerá en cada tipo de documento al imprimir.</p>
      </div>
              ...
              <div className="flex-shrink-0">
                ...
                <button onClick={() => ref.current?.click()} disabled={isUploading}
                  ...
                  {isUploading
                    ...
                    : <><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>{url ? 'Cambiar' : 'Subir imagen'}</>
                  }
                </button>
              </div>
            </div>
