...
import { ModalFichaAnimal } from './FichaAnimalModal';
...
type FormStep = 'tutor' | 'animal' | 'resumen';
...
const ESPECIES: Record<string, { label: string; emoji: string; borde: string; razas: string[] }> = {
  DOG:   { label: 'Perro',  emoji: '🐕', borde: 'border-amber-300 bg-amber-50 text-amber-700',
           razas: ['Mestizo','Labrador','Golden Retriever','Bulldog','Beagle','Pastor Alemán','Poodle','Chihuahua','Shih Tzu','Schnauzer','Otro'] },
...
const EMPTY_FORM: Omit<Animal,'id'> = {
  ...
  tutor: { nombre: '', telefono: '', email: '', direccion: '', documento: '' },
  foto: '',
};
...
function ModalAnimal({
  ...
  const [step, setStep] = useState<FormStep>('tutor');
  ...
  const setTutor = (f: Partial<Tutor>) => setForm(p => ({ ...p, tutor: { ...p.tutor, ...f } }));

  const espInfo = ESPECIES[form.especie] ?? ESPECIES.OTHER;

  const canNextProp = !!form.tutor.nombre && !!form.tutor.telefono;
  ...
  const steps: FormStep[] = ['tutor', 'animal', 'resumen'];
  const stepIdx = steps.indexOf(step);
  const stepLabels = ['Tutor', 'Mascota', 'Resumen'];
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[95vh]">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          ...
          {step === 'tutor' && (
            <div className="space-y-3">
              <p className="text-sm font-semibold text-gray-700 mb-3">Datos del tutor</p>
              <div className="grid grid-cols-2 gap-4">
                ...
                <Field label="RUT / DNI / Documento" required>
                  <input value={form.tutor.documento} onChange={e => setTutor({ documento: e.target.value })}
                    placeholder="Ej: 12.345.678-9" className={inputCls} />
                </Field>
                <Field label="Teléfono" required>
                  <input value={form.tutor.telefono} onChange={e => setTutor({ telefono: e.target.value })}
                    placeholder="+56 9 1234 5678" className={inputCls} type="tel" />
                </Field>
                <Field label="Correo electrónico">
                  <input value={form.tutor.email} onChange={e => setTutor({ email: e.target.value })}
                    placeholder="correo@ejemplo.com" className={inputCls} type="email" />
                </Field>
                <Field label="Dirección">
                  <input value={form.tutor.direccion} onChange={e => setTutor({ direccion: e.target.value })}
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <Field label="Nº microchip">
                  <input value={form.microchip} onChange={e => set({ microchip: e.target.value })}
                    placeholder="15 dígitos" className={inputCls} maxLength={15} />
                </Field>
              ...
              <Field label="Alergias conocidas">
                <textarea value={form.alergias} onChange={e => set({ alergias: e.target.value })}
                  rows={2} placeholder="Ej: Polen, penicilina… (dejar en blanco si no hay)"
              ...
              <Field label="Condiciones preexistentes / Observaciones">
                <textarea value={form.condiciones} onChange={e => set({ condiciones: e.target.value })}
                  rows={2} placeholder="Ej: Artritis crónica, operado de cadera…"
                  className={`${inputCls} resize-none`} />
              </Field>
            </div>
          )}
          ...
          {step === 'resumen' && (
            <div className="space-y-5">
              <p className="text-sm font-semibold text-gray-700">Revisa la información antes de guardar</p>
              ...
              <div className="flex items-center gap-4 p-4 bg-emerald-50 border border-emerald-200 rounded-xl">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">{espInfo.emoji} {espInfo.label} · {form.raza || 'Raza no especificada'}</p>
                  <p className="text-xs text-gray-400 mt-0.5">
                    {form.sexo === 'M' ? '♂ Macho' : form.sexo === 'F' ? '♀ Hembra' : ''}
                    {form.peso ? ` · ${form.peso} kg` : ''}
                    {form.fechaNacimiento ? ` · Nac. ${form.fechaNacimiento}` : ''}
                  </p>
                </div>
              </div>
              ...
              <div className="bg-gray-50 rounded-xl p-4 space-y-1.5 border border-gray-100">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Tutor</p>
                <Row label="Nombre"  val={form.tutor.nombre} />
                <Row label="Documento" val={form.tutor.documento} />
                <Row label="Teléfono"  val={form.tutor.telefono} />
                {form.tutor.email    && <Row label="Email"     val={form.tutor.email} />}
                {form.tutor.direccion && <Row label="Dirección" val={form.tutor.direccion} />}
              </div>
        ...
        <div className="flex gap-2 px-6 py-4 border-t border-gray-100 flex-shrink-0">
          ...
          {step !== 'resumen' ? (
            <button
              onClick={() => setStep(steps[stepIdx + 1])}
              disabled={step === 'tutor' ? !canNextProp : !canNextAnimal}
            ...
            <button
              onClick={() => onSave(form)}
              disabled={saving}
              className="inline-flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-semibold bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-50 transition-colors shadow-sm"
            >
...
export function AnimalesPage() {
  ...
  const [fichaAnimal, setFichaAnimal] = useState<Animal | null>(null);
  const [search, setSearch] = useState('');
  const [filtroEspecie, setFiltroEspecie] = useState('');

  async function guardar(form: Omit<Animal,'id'>) {
    setSaving(true);
    try {
      if (editAnimal) {
        const { data } = await apiClient.put(`/animales/${editAnimal.id}`, form);
        setAnimals(p => p.map(a => a.id === editAnimal.id ? data : a));
      } else {
        const { data } = await apiClient.post('/animales', form);
        setAnimals(p => [data, ...p]);
      }
      setShowModal(false);
      setEditAnimal(null);
  ...
  const filtrados = animals.filter(a => {
    ...
    const matchSearch = !q || a.nombre.toLowerCase().includes(q)
      ...
      || a.tutor.nombre.toLowerCase().includes(q);
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">Fichas de pacientes de la clínica</p>
        </div>
        <button
          onClick={abrirNuevo}
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="grid grid-cols-12 gap-4 px-5 py-3 bg-gray-50 border-b border-gray-100 text-xs font-semibold text-gray-500 uppercase tracking-wider">
          <span className="col-span-4">Animal / Tutor</span>
        ...
        {loading ? (
          <div className="flex items-center justify-center py-14">
            ...
            <span className="ml-3 text-sm text-gray-400">Cargando pacientes...</span>
          </div>
        ) : filtrados.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-14 text-center">
            ...
            <p className="text-xs text-gray-400 mt-1">
              {search || filtroEspecie ? 'Intenta cambiar los filtros' : 'Agrega el primer paciente con el botón superior'}
            </p>
          </div>
        ) : (
          filtrados.map(a => {
              ...
              <div key={a.id} className="grid grid-cols-12 gap-4 px-5 py-4 items-center border-b border-gray-50 hover:bg-emerald-50/40 transition-colors">
                <div className="col-span-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">👤 {a.tutor.nombre}</p>
                  </div>
                </div>
                ...
                <div className="col-span-2 flex justify-end gap-1">
                  <button
                    ...
                    className="text-xs text-white bg-emerald-600 hover:bg-emerald-700 font-medium px-2.5 py-1.5 rounded-lg transition-all flex items-center gap-1"
                    title="Ver ficha e historial clínico"
                  >
                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  ...
                  <button
                    ...
                    className="text-xs text-red-400 hover:text-white hover:bg-red-500 font-medium px-2 py-1.5 rounded-lg border border-red-200 hover:border-red-500 transition-all"
                    title="Eliminar"
                  >
                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
